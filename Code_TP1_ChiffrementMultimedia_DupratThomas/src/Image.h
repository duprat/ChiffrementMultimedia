...
public:
    ...
    {
        std::vector<int> gray_scale;
        std::vector<float> probas;
        float sum = 0.f;

        gray_scale.push_back(abs(get(0)->getValue()));

        for (int i = 1; i < getSize(); i++)
        {
            bool contains = false;
            for(int j = 0; j < gray_scale.size();j++){
                if(abs(gray_scale[j]) == abs(get(i)->getValue())){
                    contains = true;
                    break;
                }
            }
            ...
            if(!contains){
                gray_scale.push_back(abs(get(i)->getValue()));
            }
        }
        ...
        for (int i = 0; i < gray_scale.size(); i++)
        {
            probas[i] /= (float)getSize();
        }
        
        for (int i = 0; i < gray_scale.size(); i++)
        {
            sum += (probas[i] * log(probas[i]));
        }
